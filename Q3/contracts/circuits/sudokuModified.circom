...
template sudoku() {
    ...
    component solutionPuzzle[9][9];

    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            rangePuzzle[i][j] = RangeProof(4);
            rangePuzzle[i][j].range[0] <== 0;
            rangePuzzle[i][j].range[1] <== 9;
            rangePuzzle[i][j].in <== puzzle[i][j];
            rangePuzzle[i][j].out === 1;

            solutionPuzzle[i][j] = RangeProof(4);
            solutionPuzzle[i][j].range[0] <== 0;
            solutionPuzzle[i][j].range[1] <== 9;
            solutionPuzzle[i][j].in <== solution[i][j];
            solutionPuzzle[i][j].out === 1;
